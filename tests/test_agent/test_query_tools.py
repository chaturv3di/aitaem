...
def test_compute_metrics_result_is_tool_result():
    assert issubclass(Q_ComputeMetricsResult, ToolResult)


def test_all_result_models_are_tool_results():
    for cls in [RankByValueResult, FilterByThresholdResult, PeriodOverPeriodResult, ContributionShareResult]:
...
def test_resolve_intent_uses_exact_match_not_raw_arguments():
    """Regression test (Plan 37, SF-3): resolve_intent must build ResolvedSpec and
    the returned ExactMatch from SpecResolver.resolve()'s exact_match, not the raw
    metric_name/slices/segment arguments — resolver.py's own documented contract
    (09-querybot-v0.2-design.md §4.2). Patches resolve() to return an exact_match
    that diverges from the arguments passed in, and asserts every downstream use
    (spec_registry's ResolvedSpec, the returned ExactMatch, and the
    column_distribution_result_id cross-metric-name check) follows exact_match."""
    from aitaem.agent.resolver import ExactMatch, SpecMatchResult

    deps = _make_deps()
    deps.spec_cache.metrics["canonical_revenue"] = deps.spec_cache.metrics["revenue"]
    ctx = _make_ctx(deps)
    record_intent(ctx, metric_concept="revenue", scope="overall")
    ...
    with patch("aitaem.agent.query_tools.SpecResolver") as mock_resolver_cls:
    ...
    assert result.exact_match is not None
    assert result.exact_match.metric_name == "canonical_revenue"
    assert result.exact_match.slices == ["by_country"]
    assert result.exact_match.segment is None

    token = result.exact_match.spec_token
    resolved = deps.spec_registry[token]
    assert resolved.metric_name == "canonical_revenue"
    assert resolved.slice_specs == ["by_country"]


def test_resolve_intent_column_distribution_mismatch_check_uses_exact_match():
    """Same regression as above, specifically for the column_distribution_result_id
    cross-metric-name check: it must compare against exact_match.metric_name, not
    the raw metric_name argument."""
    from aitaem.agent.resolver import ExactMatch, SpecMatchResult
    ...
    diverging_match = ExactMatch(
        spec_token="", metric_name="canonical_revenue", slices=[], segment=None,
    )
    with patch("aitaem.agent.query_tools.SpecResolver") as mock_resolver_cls:
        mock_resolver_cls.return_value.resolve.return_value = SpecMatchResult(
            exact_match=diverging_match, near_misses=[],
        )
        # Argument says "revenue" (matches the stored column_distribution's
        # metric_name), but exact_match diverges to "canonical_revenue" — the
        # mismatch check must fire on exact_match.metric_name, catching this.
        result = resolve_intent(ctx, intent_id=0, metric_name="revenue")

    assert result.exact_match is None
    assert any(
        nm.why_not == "column_distribution_metric_mismatch" for nm in result.near_misses
    )
